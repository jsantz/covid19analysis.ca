...
function play(b){sample--;healthys.forEach(function(a){a.y+=a.vy;a.x+=a.vx;var b=contain(a,{x:28,y:10,width:800,height:800});if("top"===b||"bottom"===b)a.vy*=-1;if("left"===b||"right"===b)a.vx*=-1;healthys.forEach(function(b){hitTestRectangle(a,b,Radius)&&bump&&(a.vx*=-1,a.vy*=-1,b.vx*=-1,b.vy*=-1)})});infecteds.forEach(function(a,b,d){a.y+=a.vy;a.x+=a.vx;var c=contain(a,{x:28,y:10,width:800,height:800});if("top"===c||"bottom"===c)a.vy*=-1;if("left"===c||"right"===c)a.vx*=-1;infecteds.forEach(function(b){hitTestRectangle(a,
    ...
    y:perc(mortalset[timeline].y+immuneset[timeline].y+infectedset[timeline].y,mortalset[timeline-1].y+immuneset[timeline-1].y+infectedset[timeline-1].y)},window.myLineP.update()),window.myLine.update(),timeline++)}function perc(b,a){return 100*(b-a)}function end(){}function transmission(){return Math.random()<transmissibility/100}function mortality(){return Math.random()<mortalityRate/100}
function contain(b,a){var c=void 0;b.x<a.x&&(b.x=a.x,c="left");b.y<a.y&&(b.y=a.y,c="top");b.x+b.width>a.width&&(b.x=a.width-b.width,c="right");b.y+b.height>a.height&&(b.y=a.height-b.height,c="bottom");return c}
...
$("#runsimulation").click(function(){bump=$("#isBounce").is(":checked");populations=$("#slider-range-min").slider("value");transmissibility=$("#transmissibility").slider("value");socialSpeed=$("#movementspd").slider("value")/100;quarantineLevel=$("#shelter").slider("value");Radius=$("#radius").slider("value");infectionLength=30*$("#infectionTime").slider("value");mortalityRate=$("#mortality").slider("value");riskpop=$("#amountinf").slider("value");naturalImmunity=$("#amountimm").slider("value");mortalityRate=
    $("#mortality").slider("value");app.destroy(!0);app=null;loader.reset();app=new Application({width:810,height:810,antialiasing:!0,transparent:!1,resolution:1});healthyset=[];immuneset=[];infectedset=[];mortalset=[];healthyperc=[];immuneperc=[];infectedperc=[];mortalperc=[];xvalsSim=[];timeline=0;window.myLine.update();window.myLineP.update();window.myLine.destroy();window.myLineP.destroy();$("#graphholder").html("<h1>Resetting...</h1>");var b={type:"line",data:{labels:xvalsSim,datasets:[{label:"Victim",
    ...
    $("#graphholder").html('<canvas id="canvasperc"></canvas><canvas id="canvassim"></canvas>');var c=document.getElementById("canvassim").getContext("2d");window.myLine=new Chart(c,b);b=document.getElementById("canvasperc").getContext("2d");window.myLineP=new Chart(b,a);renderScene()});
$(function(){var b=$("#custom-handle");$("#slider-range-min").slider({range:"min",value:200,min:50,max:800,create:function(){b.text($(this).slider("value"))},slide:function(a,c){$("#amount").val(c.value);b.text(c.value)}});$("#amount").val(+$("#slider-range-min").slider("value"))});
...
$(function(){var b=$("#amountinf-handle");$("#amountinf").slider({range:"min",value:1,min:0,max:100,create:function(){b.text($(this).slider("value"))},slide:function(a,c){$("#amountinf-label").val(c.value);b.text(c.value)}});$("#amountinf-label").val($("#amountinf").slider("value"))});
$(function(){var b=$("#amountimm-handle");$("#amountimm").slider({range:"min",value:1,min:0,max:400,create:function(){b.text($(this).slider("value"))},slide:function(a,c){$("#amountimm-label").val(c.value);b.text(c.value)}});$("#amountimm-label").val($("#amountimm").slider("value"))});